/* theme-switcher.js — selector de temas arrastrable, v2 */
(function () {
  'use strict';
  ...
  const STYLE = `
    ...
    /* body transition */
    body, .garden, .grass-blade, .cloud, .cloud1,.cloud2,.cloud3,.cloud4,.cloud5 {
  ...
  function build() {
    ...
    el.innerHTML = `
      <div id="ts-pill">
        <div id="ts-thumb"></div>
        ${THEMES.map((t, i) => `
          <button class="ts-btn${i === current ? ' active' : ''}" data-i="${i}" title="${t.label}" aria-label="${t.label}">
  ...
  function moveThumb(idx, instant) {
    ...
    else         thumb.style.transition = 'left .36s cubic-bezier(.4,0,.2,1), width .36s cubic-bezier(.4,0,.2,1)';
  ...
  function apply(idx, animate) {
    ...
    if (rc) rc.style.display = rain ? '' : 'none';
    if (lt) { lt.style.display = rain ? '' : 'none'; lt.style.opacity = '0'; }


    if (rain && !stormActive) startStorm();
    if (!rain) { stormActive = false; if (stormTimer) clearTimeout(stormTimer); stormTimer = null; }


    document.querySelectorAll('.puddle').forEach(p => p.style.display = rain ? '' : 'none');


    const oc = document.querySelector('.overcast');
    if (oc) oc.style.opacity = rain ? '1' : '0';
  ...
  function startStorm() {
    ...
    function tick() {
      ...
      if (lt && lt.style.display !== 'none') {
        lt.style.opacity = '0.88';
        setTimeout(() => lt && (lt.style.opacity = '0'),   70);
        setTimeout(() => lt && (lt.style.opacity = '0.55'),135);
        setTimeout(() => lt && (lt.style.opacity = '0'),   210);
        setTimeout(() => lt && (lt.style.opacity = '0.22'),290);
        setTimeout(() => lt && (lt.style.opacity = '0'),   370);
      }
      stormTimer = setTimeout(tick, 6000 + Math.random()*12000);
    }
    stormTimer = setTimeout(tick, 1800 + Math.random()*3000);
  }
